...
namespace Pharmacy
{
    ...
    public class TableEditWindow<T> : VBox where T : class
    {
        ...
        protected PharmContext pharmContext;
        ...
        public TableEditWindow(PharmContext pharmContext, DbSet<T> dbset, ErrorsWindow errorsWindow, int rows, List<EntryType> entryTypes, List<string> defaultValues) : base(false, 6)
        {
            this.pharmContext = pharmContext;
            this.dbset = dbset;
        ...
        protected virtual void AddRow(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < table.columns; i++)
            {
                if (i < entryTypes.Count)
                {
                    switch (entryTypes[i])
                    {
                        ...
                        case EntryType.NonIncrementedId:
                            ...
                            break;
                        default:
                            if (defaultValues != null && i < defaultValues.Count)
                            {
                ...
                {
                    // Використовуйте значення за замовчуванням, якщо entryTypes не має відповідного індексу
                    values.Add("");
                }
            }
            table.AddRow(values);
